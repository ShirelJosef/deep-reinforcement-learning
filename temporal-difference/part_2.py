...
def get_expected_Q(state, Q, epsilon, nA):
    probabilities = epsilon_greedy(state, Q, epsilon, nA)
    expected_Q = np.sum(probabilities * Q[state])
    return expected_Q
...
def expected_sarsa(env, num_episodes, alpha, gamma=1.0):
    ...
    for i_episode in range(1, num_episodes +1):
        ...
        while True:
            ...
            Q_expected = get_expected_Q(next_state, Q, epsilon, env.action_space.n)
            Q[state][action] = Q[state][action] + alpha * (
                    reward + gamma * Q_expected - Q[state][action])
            state = next_state
            action = next_action
            if done:
                break
