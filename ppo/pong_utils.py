...
def animate_frames(frames):
    ...
    for i in range(len(frames)):
        im = plt.imshow(frames[i], animated=True)#, cmap=cmap)
        #plt.show()
        ims.append([im])
    patch = plt.imshow(frames[0], cmap=cmap)  
       
    #fanim = animation.ArtistAnimation(plt.gcf(),ims,interval=50,blit=True,repeat_delay=1000)
    fanim = animation.FuncAnimation(plt.gcf(), lambda x: patch.set_data(frames[x]), frames = len(frames),interval=40)
    
    #HTML(fanim.to_jshtml())
    return fanim
    #plt.show()
    #fanim = animation.FuncAnimation(plt.gcf(), \
    #    lambda x: patch.set_data(frames[x]), frames = len(frames), interval=30,blit=True)
    #display(fanim)
